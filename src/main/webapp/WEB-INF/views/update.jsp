...
<c:forEach var="s" items="${snsAl }">
	<c:if test="${modify == s.js_no }">
		<form action="update.sns" method="post" name="updateForm" onsubmit="return updateCheck();">
	</c:if>
	...
	<table class="snsListTable pad">
		...
		<tr>
			<td colspan="3" style="padding-bottom: 10px;">
				<c:choose>
					<c:when test="${modify == s.js_no }">
						<textarea name="js_txt" class="wInput" rows="2" >${s.js_txt }</textarea>
					</c:when>
					<c:otherwise>
						${s.js_txt }
					</c:otherwise>
